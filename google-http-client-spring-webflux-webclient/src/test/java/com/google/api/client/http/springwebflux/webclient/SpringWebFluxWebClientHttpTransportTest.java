...
/**
 * Tests {@link SpringWebFluxWebClientHttpTransport}.
 *
 * @author Yaniv Inbar
 */
@Ignore
public class SpringWebFluxWebClientHttpTransportTest {
    ...
    public void testApacheHttpTransport() {
        SpringWebFluxWebClientHttpTransport transport = new SpringWebFluxWebClientHttpTransport();
    ...
    public void testApacheHttpTransportWithParam() {
        SpringWebFluxWebClientHttpTransport transport = new SpringWebFluxWebClientHttpTransport(SpringWebFluxWebClientHttpTransport.newDefaultHttpClient(),
                                                                                                true);
        checkHttpTransport(transport);
    ...
    public void testNewDefaultHttpClient() {
        var client = SpringWebFluxWebClientHttpTransport.newDefaultHttpClient();
        checkHttpClient(client);
    }

    private void checkHttpTransport(SpringWebFluxWebClientHttpTransport transport) {
        assertNotNull(transport);
        var client = transport.getWebClient();
        checkHttpClient(client);
    }

    private void checkHttpClient(WebClient client) {
    ...
    private void subtestUnsupportedRequestsWithContent(SpringWebFluxWebClientHttpRequest request, String method) throws IOException {
        try {
            execute(request);
            ...
            // expected
            assertEquals(e.getMessage(), "Apache HTTP client does not support " + method + " requests with content.");
        }
    }

    private void execute(SpringWebFluxWebClientHttpRequest request) throws IOException {
    ...
    public void testRequestShouldNotFollowRedirects() throws IOException {
        ...
        SpringWebFluxWebClientHttpTransport transport = new SpringWebFluxWebClientHttpTransport(SpringWebFluxWebClientHttpTransport.newDefaultHttpClient());
        SpringWebFluxWebClientHttpRequest request = transport.buildRequest("GET", "https://google.com");
    ...
    public void testConnectTimeout() {
        ...
        HttpTransport httpTransport = new SpringWebFluxWebClientHttpTransport();
    ...
    public void testNormalizedUrl() throws IOException {
        ...
        try (FakeServer server = new FakeServer(handler)) {
            HttpTransport transport = new SpringWebFluxWebClientHttpTransport();
            GenericUrl testUrl = new GenericUrl("http://localhost/foo//bar");
            testUrl.setPort(server.getPort());
            com.google.api.client.http.HttpResponse response = transport.createRequestFactory().buildGetRequest(testUrl).execute();
